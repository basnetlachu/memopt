...
class ApplyEngine:
    ...
    def apply(
        ...
        Args:
            ...
            mode:     "auto" | "batch" | "turbo"
                      "turbo"       — generate memopt Turbo Engine server script
                      "draft"       — generate draft acceleration wrapper
                      "turbo+draft" — both combined
        ...
        try:
            ...
            if mode == "turbo+draft":
                ...
                print("\n  Both scripts generated. For maximum throughput use --mode turbo.")
                print("  Stack draft acceleration on top with --mode draft for minimum latency.")
                return ApplyResult(
    ...
    def _apply_vllm(self, profile: ProcessProfile) -> ApplyResult:
        ...
        script_path = f"/tmp/memopt_turbo_{profile.pid}.py"

        script = (
            "#!/usr/bin/env python3\n"
            "# ============================================================\n"
            "# Generated by memopt\n"
            "# Technique: Continuous Batching Engine (Turbo mode)\n"
            f"# Model:     {model_path}\n"
            ...
            "# Expected:   ~5-6x throughput vs single-request baseline\n"
            "# ============================================================\n"
            "import subprocess, sys, os\n"
            "\n"
            f"MODEL = \"{model_path}\"\n"
            f"HOST  = os.getenv(\"MEMOPT_HOST\", \"0.0.0.0\")\n"
            f"PORT  = int(os.getenv(\"MEMOPT_PORT\", \"8001\"))\n"
            "\n"
            f"print(f\"[memopt-turbo] Model:     {{MODEL}}\")\n"
            f"print(f\"[memopt-turbo] Precision: float16 (no quantization)\")\n"
            f"print(f\"[memopt-turbo] Max seqs:  {max_seqs}\")\n"
            f"print(f\"[memopt-turbo] Batching:  continuous\")\n"
            f"print(f\"[memopt-turbo] API:       http://{{HOST}}:{{PORT}}/v1\")\n"
            "\n"
            "cmd = [\n"
        ...
        print()
        print(f"  memopt Turbo Engine script  →  {script_path}")
        ...
        self._post_event(profile, ["turbo_continuous_batching"])
        ...
        return ApplyResult(
            ...
            success=True,
            rolled_back=False,
            optimizations_applied=["turbo_continuous_batching"],
    ...
    def _apply_speculative(self, profile: ProcessProfile) -> ApplyResult:
        ...
        script = (
            ...
            "\n"
            "print(f\"[memopt-draft] Loading main model: {MAIN_MODEL}\")\n"
            ...
            ")\n"
            "main_model.eval()\n"
            "print(f\"[memopt-draft] Main model loaded\")\n"
            "\n"
            "print(f\"[memopt-draft] Loading draft accelerator...\")\n"
            ...
            ")\n"
            "draft_model.eval()\n"
            "print(f\"[memopt-draft] Draft acceleration ready\")\n"
            "\n"
            "\n"
            ...
            "    print(\"[memopt-draft] Test: generating 50 tokens...\")\n"
            "    result = generate(\"Explain the benefits of GPU optimization:\", max_new_tokens=50)\n"
            "    print(result)\n"
            "    print(\"[memopt-draft] Done. Import this module and call generate(prompt).\")\n"
        )
        ...
        print()
        print(f"  memopt Draft Acceleration script  →  {script_path}")
        print(f"  Main model  : {model_path}")
        print(f"  Precision   : float16  (no quantization)")
        print(f"  Expected    : 1.5-2.5x latency improvement")
        print(f"  Quality     : identical — outputs mathematically verified")
        print()
        print("  Run:")
        print(f"    python3 {script_path}")
