...
class MLIRStrategy:
    ...
    def compile(self, source: str) -> Optional[types.ModuleType]:
        ...
        try:
            ...
            func_name = f"kernel_{stamp}_{uid}"
            path      = os.path.join(out_dir, f"{func_name}.mlir")

            # Emit real MLIR text dialect.
            # The Triton Python source is preserved as comments so the vendor
            # toolchain can reconstruct intent; the wrapper satisfies the MLIR
            # parser's module/func.func grammar.
            commented = "\n".join(f"  // {line}" for line in source.splitlines())
            mlir_text = (
                f'// Auto-generated by memopt MLIRStrategy\n'
                f'// Device: {os.environ.get("MEMOPT_MLIR_DEVICE", "custom_asic")}\n'
                f'\n'
                f'module @{func_name} {{\n'
                f'  func.func @run_kernel() -> () {{\n'
                f'    // --- Triton source (reference) ---\n'
                f'{commented}\n'
                f'    // --- end Triton source ---\n'
                f'    return\n'
                f'  }}\n'
